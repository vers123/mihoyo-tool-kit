...
新增教程更新日志页面的批量抓取功能，支持从索引页提取所有教程链接并逐个抓取教程详情页。

### Added
- **批量抓取教程目录**：输入更新日志索引页 ID（如 `mhs2w008wf14`），自动提取所有教程详情页链接，逐个抓取并保存为 HTML。
  - 已存在的教程页面自动跳过，避免重复抓取。
  - CLI 菜单 23「批量抓取教程目录」+ GUI「批量抓取目录」按钮。
- **ChangelogExtractor**：新增 `extract_all_links()` 方法，从更新日志 HTML 中提取所有教程链接（去重）。
- **教程提取自动识别**：`run_extract_tutorial` 自动检测页面类型（角色编号 / 更新日志），路由到对应提取器。

### Changed
- CLI 菜单编号重排：新增 23「批量抓取教程目录」，原 23-39 顺移至 24-41。
- GUI「其他抓取」页面新增「批量抓取目录」按钮，「提取角色」按钮改名为「提取数据」。
