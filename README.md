...
| 23 | 批量抓取教程目录 | 抓取更新日志索引页，提取所有链接并逐个抓取教程详情页 |
| 24 | 提取教程数据 | 从教程页面提取角色数据或更新日志（自动识别） |
| 25 | 提取图鉴图片链接 | 从抓取的图鉴页面提取角色图片链接 |
| 26 | 抓取自定义网站 | 抓取任意网站的HTML页面（需输入URL） |

#### 微博（CLI 27-30）

| 序号 | 功能 | 说明 |
| :---: | ------ | ------ |
| 27 | 抓取微博用户主页 | 从微博抓取指定用户的发帖记录 |
| 28 | 增量抓取微博用户 | 增量更新微博用户发帖，自动备份旧数据 |
| 29 | 提取微博数据 | 从微博页面提取发帖时间和内容 |
| 30 | 增量提取微博数据 | 增量提取并合并新旧微博数据 |

#### 系统工具（CLI 31-38）

| 序号 | 功能 | 说明 |
| :---: | ------ | ------ |
| 31 | 查看备份文件 | 查看所有数据备份文件 |
| 32 | 恢复备份数据 | 从备份文件恢复数据 |
| 33 | 查看当前配置 | 显示当前的配置信息 |
| 34 | 修改配置参数 | 修改URL、超时时间等配置 |
| 35 | 重新加载配置 | 从配置文件重新加载配置 |
| 36 | 系统信息 | 显示系统环境和依赖信息 |
| 37 | 数据迁移工具 | 迁移旧版本数据到新目录结构 |
| 38 | 清理缓存文件 | 清理 __pycache__、日志、构建临时文件（需二次确认） |
| 0 | 退出程序 | 退出米游社工具箱 |
...
| 序号 | 功能 | 说明 |
| :---: | ------ | ------ |
| 39 | 导出新闻到 Excel | 从 SQLite 导出四站点新闻到 .xlsx（每站点一 sheet） |
| 40 | 导出 RSS/JSON Feed | 从 SQLite 生成 RSS/JSON feed 供外部订阅 |
| 41 | 过滤 TXT 文件 | 按关键词匹配提取行，按时间升序/降序重新编号（支持精准/模糊匹配） |
...
│   ├── weibo.py               # 微博数据
│   ├── images.py              # 图片链接
│   ├── tutorial.py            # 教程数据（角色编号提取 + 更新日志解析）
...
**V1.3.0** · Licensed under [MIT](LICENSE) · Maintained by LingLan · [Changelog](CHANGELOG.md)
